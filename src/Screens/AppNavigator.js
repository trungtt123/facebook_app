...
export default function AppNavigator() {
    ...
    const { isLoading, isAuthenticated, loginWithCache } = useSelector(
        (state) => state.auth
    );
    ...
    if ((token !== undefined && token !== null && token !== "" && !netInfo.isConnected) || isAuthenticated) return <>
        <NavigationContainer>
            <Stack.Navigator>
                {!loginWithCache && <Stack.Screen name="saveLoginInfo" component={SaveLoginInfoScreen} options={{ title: 'Lưu thông tin đăng nhập' }} />}
